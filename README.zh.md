...
iframe 池会保留每个分组的活动 Tab，以及最近使用的 2 个非活动 Tab；被淘汰的 Tab 再次打开时会重新加载。共享预览按需为通过选择器访问过的每个工作区保留一个独立 iframe，保留各工作区的原生标签，直到页面刷新。分组越多，浏览器内存和 DSH Web 连接数越高。

整页共用一个右侧预览栏；切换会话或分组不会切换或重置已打开的文件。任意会话中的文件链接都在此处打开，收起后保留预览内容。“文件”标签将所有工作区列为独立根目录，按需展开，可同时浏览多个项目；切换会话不会改变文件树或已打开的预览。文件树和预览状态在当前页面内保留，刷新页面后重置。通过右栏顶部的工作区选择器，明确选择终端、Git 等会话绑定工具所属的工作区；切换活动会话不会改变此选择，手动切回工作区时恢复其原生标签。会话底部面板仍属于各自的 Session。已归档 Session 和子 Agent 路由使用原生对话视图；触屏布局使用拆分按钮和分组切换栏，不依赖 Tab 拖拽。
...
已验证 `dsh-better-sidebar@0.19.0`：共享文件树支持多个工作区根目录，右栏终端和 Git 使用明确选择的工作区，会话底部工作台可正常展开。
